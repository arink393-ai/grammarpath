...
function dailyQuestion(){
 ...
 document.querySelector('#dq-next').onclick=()=>{if(!DQ.checked)return;DQ.index++;if(DQ.index>=d.questions.length)dailyFinish();else dailyQuestion();};
...
function dailyCheck(value){
 if(!DQ||DQ.checked||AUTH.current()?.email!==DQ.owner)return;
 const q=DQ.d.questions[DQ.index],choice=q[0]==='pick'||q[0]==='sort',answer=q[choice?3:2],explanation=q[choice?4:3];
...
function dailyFinish(){
 if(!DQ||AUTH.current()?.email!==DQ.owner||!DQ.d.questions.every((q,i)=>DQ.answers[i]))return;
